...
DRIVE = {"t": 0.0, "stage": "enter", "start": None}
QA_RESULT = "RUNNING"
...
def _step(dt):
    ...
    if DRIVE["stage"] == "enter":
        ...
        out = pc.get_controlled_pawn() == walker and not walker.is_hidden()
        ok = DRIVE["dist"] > 5 and out
        return _finish(f"{'PASS' if ok else 'FAIL'} drove {DRIVE['dist']:.1f} m in 6 s, got back out: {out}")
